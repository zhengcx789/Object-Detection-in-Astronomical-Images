function [Dist,RSS, XYproj,di] = Residuals_ellipse(XY,ParG)
...
% Residuals_ellipse.m:                                       %
%                                                          %
% Copyright (c) 2010, Hui Ma, All rights reserved.         %
%                                                          %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if  abs((Axes(1)-Axes(2))/Axes(1))<tolerance
    phiall = angle(XY(:,1)-Center(1) + sqrt(-1)*(XY(:,2)-Center(2)));   %arctan (sqrt(-1)*(XY(:,2)-Center(2)))/(XY(:,1)-Center(1)),ÖµÓòÎª£¨-pi¡ª¡ªpi£©
    XYproj = [Axes(1)*cos(phiall)+Center(1) Axes(2)*sin(phiall)+Center(2)];
    RSS = norm(XY-XYproj,'fro')^2;   
    di=XY-XYproj; 
    d=(XY-XYproj)*(XY-XYproj)';
    Dist=sqrt(diag(d)); 
    return;
end
